...
function Creepr1:init(map)
    ...
    self.behaviors = {
       ...
       ['jumping'] = function(dt)            
            if self.dy > 0 then
                if self.dx == 0 and self.x > map.mapWidth - map.tileWidth * 2 then
                    self.direction = 'left'
                    self.dx = -WALKING_SPEED
                elseif self.dx == 0 and self.x < map.tileWidth * 3   then
...
function Creepr1:calculateJumps()
...
function Creepr1:checkLeftCollision()
    if self.dx < 0 then
        -- check if there's a tile directly beneath us
...
function Creepr1:checkRightCollision()
    if self.dx > 0 then
        -- check if there's a tile directly beneath us
