...
function Slime:init(map)    
   ...
   animation =  newAnimationslime(love.graphics.newImage("graphics/slime.png"),16,16,1.25)
  
     
   
end

 function newAnimationslime(image, width, height, duration)

    local animation = {}
    animation.spriteSheet = image;
    animation.quads = {};

    table.insert(animation.quads, love.graphics.newQuad(0 , 0, 16, 16, image:getDimensions()))
    table.insert(animation.quads, love.graphics.newQuad(16, 0, 16, 16, image:getDimensions())) 
    table.insert(animation.quads, love.graphics.newQuad(32, 0, 16, 16, image:getDimensions()))     
...
function Slime:update(dt)
    ...
    if self.y <= map.tileHeight * (map.mapHeight - 7) - self.height  then
...
function Slime:render()
    ...
    love.graphics.draw(animation.spriteSheet, animation.quads[spriteNum], math.floor(self.x + self.xOffset), 
        math.floor(self.y + self.yOffset) , 4.65, scaleX, 1, self.xOffset, self.yOffset)
